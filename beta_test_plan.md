...
| F4 | Developer | Callable function for shader implementation | Each shader will have a function to be called and modified |
...
| F15 | Developer | Use atmospheric scattering shader | Easy-to-implement shader that adds atmospheric scattering to a position in the world. |
...
| F3 | A user can use the Shimera library on Raylib |  | — |
| F4 | A user can implement and modify a shader using a function |  | — |
| F5 | A user can use the distortion postprocessing  shader |  | — |
| F6 | A user can use the color shift postprocessing  shader |  | — |
| F7 | A user can use the contrast postprocessing shader |  | — |
| F8 | A user can use the saturation postprocessing shader |  | — |
| F9 | A user can use brightness postprocessing shader |  | — |
| F10 | A user can use the vignette postprocessing shader |  | — |
| F11 | A user can use the chromatic aberration postprocessing shader |  | — |
| F12 | A user can use the Gaussian blur postprocessing shader |  | — |
| F13 | A user can use the pixelation postprocessing shader |  | — |
| F14 | A user can use HDR bloom postprocessing shader |  | — |
| F15 | A user can use the atmospheric scattering shader |  | — |
| F16 | A user can use Fresnel Effect spatial shader |  | — |